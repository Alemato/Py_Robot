...
def callback(msg, args):
    ...
    # lidar ogni 10 gradi
    prolog.assertz("lidar(0, " + lidar[0] + ")")
    prolog.assertz("lidar(10, " + lidar[1] + ")")
    prolog.assertz("lidar(20, " + lidar[2] + ")")
    prolog.assertz("lidar(30, " + lidar[3] + ")")
    prolog.assertz("lidar(40, " + lidar[4] + ")")
    prolog.assertz("lidar(50, " + lidar[5] + ")")
    prolog.assertz("lidar(60, " + lidar[6] + ")")
    prolog.assertz("lidar(70, " + lidar[7] + ")")
    prolog.assertz("lidar(80, " + lidar[8] + ")")
    prolog.assertz("lidar(90, " + lidar[9] + ")")
    prolog.assertz("lidar(100, " + lidar[10] + ")")
    prolog.assertz("lidar(110, " + lidar[11] + ")")
    prolog.assertz("lidar(120, " + lidar[12] + ")")
    prolog.assertz("lidar(130, " + lidar[13] + ")")
    prolog.assertz("lidar(140, " + lidar[14] + ")")
    prolog.assertz("lidar(150, " + lidar[15] + ")")
    prolog.assertz("lidar(160, " + lidar[16] + ")")
    prolog.assertz("lidar(170, " + lidar[17] + ")")
    prolog.assertz("lidar(180, " + lidar[18] + ")")
    ...
    # angolo vecchio
    prolog.assertz("oldangle8(" + oldangle + ")")
    ...
    prolog.assertz("sonartru(_):- sonar(destra, A), sonar(centro, B), sonar(sinistra, C),  A > 10, B > 10, C > 10, !")

    # regola per capire quale è il sonar con la distanza maggiore
    prolog.assertz("sonar(Y) :- sonar(centro, A), sonar(destra, B), sonar(sinistra, C), D is max(A, B), X is max(D, C), sonar(Y, X),!")
    ...
    prolog.assertz("command(X):-commandolder(O), O == avanti, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonartru(_), angle8(A), oldangle8(B), S is A-B, S > 10, C is 8, command(X, C),!")

    # comando coregi a sinistra
    prolog.assertz("command(X):-commandolder(O), O == avanti, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonartru(_), angle8(A), oldangle8(B), S is B-A, S > 10, C is 9, command(X, C),!")

    # comando avanti
    prolog.assertz("command(X):- commandolder(O), O == avanti, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonartru(_), C is 3, command(X,C),!")
    prolog.assertz("command(X):- commandolder(O), O \== indietro, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonar(U), U == centro, fotocamera(F), F == centro, C is 3, command(X,C),!")
    prolog.assertz("command(X):- commandolder(O), O \== indietro,switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonar(U), U == centro, C is 3, command(X,C),!")
    prolog.assertz("commandlidar(X):-lidar(B), B < 121, B > 61, switch(_), volt(Y), Y > 11,qrcode(Q), Q == 0, C is 3, command(X,C),!")

    # comando sinistra
    prolog.assertz("command(X):- commandolder(O), O \== indietro, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonar(U), U == sinistra, fotocamera(F), F == sinistra, C is 1, command(X,C),!")
    prolog.assertz("command(X):- commandolder(O), O \== indietro, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonar(U), U == sinistra, C is 1, command(X,C),!")
    prolog.assertz("commandlidar(X):-lidar(B), B > 120,switch(_), volt(Y), Y > 11,qrcode(Q), Q == 0, C is 1, command(X,C),!")

    # comando destra
    prolog.assertz("command(X):- commandolder(O), O \== indietro, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonar(U), U == destra, fotocamera(F), F == destra, C is 2, command(X,C),!")
    prolog.assertz("command(X):- commandolder(O), O \== indietro, switch(_), volt(Y), Y > 11, qrcode(Q), Q == 0, sonar(U), U == destra, C is 2, command(X,C),!")
    prolog.assertz("commandlidar(X):-lidar(B), B < 60,switch(_), volt(Y), Y > 11,qrcode(Q), Q == 0, C is 2, command(X,C),!")
